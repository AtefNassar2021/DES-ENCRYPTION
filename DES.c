...
static void getSubKeys(const key_t key, subkey_t ks) {
    ...
    for (i = 0; i < 56; ++i) {
        pokeBit(kp, i, peekBit(key, PC_1[i] - 1));
    }
    ...
    for (i = 1; i < 17; ++i) {
        for (j = 0; j < 48; ++j) {
            pokeBit(ks[i], j, peekBit(c[i], PC_2[j] - 1));
        }
    }
}
...
void driver(const key_t key, const ubyte *message, int len) {
    ...
    char word3[]="Decoded ";
    LCD_goto_4bit(0,0);
    LCD_write_str_4bit(word3);
    LCD_goto_4bit(1,0);
 //----------------------------------------------------- 
    //remove this part if it not work 
	if(validate(buffer))
	{
		char* text = hexToAscii(buffer);
        LCD_write_str_4bit(text);
		free(text);
	}
	else
	{}
